...
router.post('/company/:companyId/edit-profile', async (req, res, next) => {
    ...
    try {
        ...
        //company
        const {description, companyName, email, password, isHeadHunter, taxId, contactPerson, yearsExp, website, phoneNumber, numberOfEmployees, country, city } = req.body;
        ...
        const salt = bcrypt.genSaltSync(saltRounds);
        const hashPass = bcrypt.hashSync(password, salt);
        const updatedCompany = await Company.findByIdAndUpdate(checkCompany, { description, addressId, sectorId, companyName, firstName, lastName, email, password: hashPass, isHeadHunter, taxId, contactPerson, yearsExp, website, phoneNumber, numberOfEmployees, city, country }, { new: true });
