...
router.post(
    ...
    async (req, res, next) => {
        ...
        const { firstName, lastName, email, password, companyName, isHeadHunter } = req.body;
...
router.post('/company/:companyId/complete-profile', async (req, res, next) => {
    ...
    try {
        ...
        const updatedCompany = await Company.findByIdAndUpdate(checkCompany, { companyName, sectorId, taxId, addressId, website, phoneNumber, numberOfEmployees, website }, { new: true })
...
router.post('/company/:companyId/edit-profile', async (req, res, next) => {
    ...
    try {
        const { companyId } = req.params;
        const checkCompany = await Company.findById(companyId);
        //console.log(checkCompany.email)
        //sectors
        ...
        //company
        const { companyName, email, password, isHeadHunter, taxId, contactPerson, yearsExp, website, phoneNumber, numberOfEmployees, country, city } = req.body;
        console.log(req.session.currentUser)

        let addressId = await Address.findByIdAndUpdate(checkCompany.addressId, { $set: { province: { provinceName, provinceCode, provinceDescription }, municipality: { municipalityCode, municipalityDescription } }, countryCode, country, provinceINEcode, municipalityINEcode, street, number, zip }, { new: true });
        ...
        const salt = bcrypt.genSaltSync(saltRounds);
        const hashPass = bcrypt.hashSync(password, salt);
        const updatedCompany = await Company.findByIdAndUpdate(checkCompany, { addressId, sectorId, companyName, firstName, lastName, email, password: hashPass, isHeadHunter, taxId, contactPerson, yearsExp, website, phoneNumber, numberOfEmployees, city, country }, { new: true });
