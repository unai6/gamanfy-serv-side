...
router.post('/user/:userId/edit-profile', async (req, res, next) => {
    ...
    try {
        ...
        if (checkUser.isCompany) {
            const salt = bcrypt.genSaltSync(saltRounds);
            const hashPass = bcrypt.hashSync(password, salt);
            let addressId = await Address.findByIdAndUpdate(checkUser.addressId, { $set: { province: { provinceName, provinceCode, provinceDescription }, municipality: { municipalityCode, municipalityDescription } }, countryCode,  provinceINEcode, municipalityINEcode, street, number, zip }, { new: true });
