...
router.post('/user/:userId/edit-profile', async (req, res, next) => {
    ...
    try {
        ...
        if (checkUser.isCompany) {
            const salt = bcrypt.genSaltSync(saltRounds);
            const hashPass = bcrypt.hashSync(password, salt);
            const isCompanyUser = await CompanyUser.findOneAndUpdate({ sectorId, addressId, companyName, contactPerson, documentType, numberOfEmployees, documentNumber, website, city, country });
            const updatedUser = await InfluencerUser.findByIdAndUpdate(checkUser, { isCompanyUser, phoneNumber, password: hashPass, email }, { new: true });

            req.session.currentUser = updatedUser;
            res.status(200).json({ updatedUser });

        } else if (checkUser.isCompany === false || !checkUser.isCompany) {
            const salt = bcrypt.genSaltSync(saltRounds);
            const hashPass = bcrypt.hashSync(password, salt);
